...
// Demonstrating the use of utility methods in AccountCalculations
Console.WriteLine("\nDemonstrating utility methods in AccountCalculations...");

// Calculate compound interest for account 1
...
Console.WriteLine($"Compound interest on account1 balance of {principal.ToString("C", CultureInfo.CurrentCulture)} at {rate * 100:F2}% for {time} year(s): {compoundInterest.ToString("C", CultureInfo.CurrentCulture)}");
...
Console.WriteLine($"Account 1 after applying interest: Balance = {account1.Balance.ToString("C", CultureInfo.CurrentCulture)}, InterestRate = {BankAccount.InterestRate:P2}, Time Period = {timePeriodYears:F2} years");
...
BankCustomer customer4 = new("Mikaela", "Lee") { FirstName = "Mikaela", LastName = "Lee" };
...
// Using object initializer
BankAccount account8 = new(customer4.CustomerId) { AccountType = "Savings" };
